...
class LandingHeader extends Component {
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    alertMessage = (nextProps.searchErrors && toast.error('Igicuruzwa ntikibonetse'));
        ...
        <div className='grid-container'>
          ...
          <div className='grid-item search'>
            <form className='search-form' onSubmit={this.handleEnterPress}>
              <span className='all'><span onClick={this.handleSubmitAll}>BYOSE</span><div className = "vertical"></div></span>
              <input
                name='searchedItem'
                type='text'
          ...
          <div className='grid-item language' style={{visibility:(!localToken)?'visible':'hidden'}}>
            <a href="https://ricainc.co.com/">Icyongereza | </a>
            <a href="https://kiny.ricainc.co.com/">Ikinyarwanda | </a>
            <a href="https://fr.ricainc.co.com/">Igifaransa</a>
          </div>
            {(!localToken)?
            (<div className='grid-item'><Link to='/signup'>Kwiyandikisha | </Link>
             <Link to='/login'>Injira</Link></div>):
             (<div className='grid-item'><Link to='/displayclientorders'> Ibyatumijwe &nbsp;&nbsp;</Link>
            <Link to='' onClick={this.handleLogout}> Gusohoka</Link></div>)}
          <div className='grid-item navbar'>
            <Link to='/'>Ahabanza</Link>
            <Link to='#'>Amashusho</Link>
            <span style={{fontWeight:'500'}} onClick={this.handleClick}>Gishinama</span>
            <Link to='/displaynews'>Amakuru</Link>
            {this.state.loggedInUser?(this.state.loggedInUser.type ==='admin'?<Link to='/addproduct'>Umuyobozi</Link>:null):null}
            <DropDown 
            visibility={visibility} 
            details={['Ibishushanyo','Igenzura']}
